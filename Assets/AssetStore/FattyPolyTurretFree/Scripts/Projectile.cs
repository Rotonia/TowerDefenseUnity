...
public class Projectile : MonoBehaviour 
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.transform.CompareTag("Creep") )
        {
            if (other.gameObject != null)
            {
                DissolveSphere fracture = other.GetComponentInChildren<DissolveSphere>();
                if (fracture != null)
                {
                    fracture.gameObject.transform.SetParent(null);
                    fracture.dissolve = true;
                }
